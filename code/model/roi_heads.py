...
import util._utils as det_utils

from torch.jit.annotations import Optional, List, Dict, Tuple
from pdb import set_trace as pause

def fastrcnn_loss(class_logits, box_regression, labels, regression_targets, class_logits_breed, labels_breed):
    ...
    labels_breed = torch.cat(labels_breed, dim=0)
    regression_targets = torch.cat(regression_targets, dim=0)

    classification_loss = F.cross_entropy(class_logits, labels)
    classification_breed_loss = F.cross_entropy(class_logits_breed, labels_breed)
    ...
    return classification_loss, box_loss, classification_breed_loss
...
class RoIHeads(torch.nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels, gt_labels_breed):
        # type: (List[Tensor], List[Tensor], List[Tensor]) -> Tuple[List[Tensor], List[Tensor]]
        matched_idxs = []
        labels = []
        labels_breed = []
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image, gt_labels_breed_in_image in zip(proposals, gt_boxes, gt_labels, gt_labels_breed):
            ...
            if gt_boxes_in_image.numel() == 0:
                ...
                labels_in_image_breed = torch.zeros(
                    (proposals_in_image.shape[0],), dtype=torch.int64, device=device
                )
            else:
                ...
                labels_in_image_breed = gt_labels_breed_in_image[clamped_matched_idxs_in_image]
                labels_in_image_breed = labels_in_image_breed.to(dtype=torch.int64)

                # Label background (below the low threshold)
                bg_inds = matched_idxs_in_image == self.proposal_matcher.BELOW_LOW_THRESHOLD
                labels_in_image[bg_inds] = 0
                labels_in_image_breed[bg_inds] = 0

                # Label ignore proposals (between low and high thresholds)
                ignore_inds = matched_idxs_in_image == self.proposal_matcher.BETWEEN_THRESHOLDS
                labels_in_image[ignore_inds] = -1  # -1 is ignored by sampler
                labels_in_image_breed[ignore_inds] = -1

            matched_idxs.append(clamped_matched_idxs_in_image)
            
            labels.append(labels_in_image)
            labels_breed.append(labels_in_image_breed)

        return matched_idxs, labels, labels_breed
    ...
    def select_training_samples(self,
        ...
        gt_labels_breed = [t["labels_breed"] for t in targets]
        ...
        matched_idxs, labels, labels_breed = self.assign_targets_to_proposals(proposals, gt_boxes, gt_labels, gt_labels_breed)
        ...
        for img_id in range(num_images):
            ...
            labels_breed[img_id] = labels_breed[img_id][img_sampled_inds]
        ...
        return proposals, matched_idxs, labels, regression_targets, labels_breed
    ...
    def postprocess_detections(self,
        ...
        pred_scores_breed = F.softmax(class_logits_breed, -1)
        # pause()
        pred_boxes_list = pred_boxes.split(boxes_per_image, 0)
        pred_scores_list = pred_scores.split(boxes_per_image, 0)
        pred_scores_list_breed = pred_scores_breed.split(boxes_per_image, 0)
        ...
        for boxes, scores, image_shape, scores_breed in zip(pred_boxes_list, pred_scores_list, image_shapes, pred_scores_list_breed):
            ...
            idx_sel = torch.arange(boxes.shape[0], device=device)
            # labels_breed = torch.arange(101, device=device) # that suxks

            labels = labels.view(1, -1).expand_as(scores)
            # labels_breed = labels_breed.view(1, -1).expand_as(scores_breed)
            ...
            scores = scores[:, 1:]
            labels = labels[:, 1:]
            # scores_breed = scores_breed[:, 1:]
            # labels_breed = labels_breed[:, 1:]

            # batch everything, by making every class prediction be a separate instance
            boxes = boxes.reshape(-1, 4)
            scores = scores.reshape(-1)
            labels = labels.reshape(-1)
            # scores_breed = scores_breed.reshape(-1)
            # labels_breed = labels_breed.reshape(-1)

            # remove low scoring boxes
            inds = torch.where(scores > self.score_thresh)[0]
            boxes, scores, labels, idx_sel = boxes[inds], scores[inds], labels[inds], idx_sel[inds]

            # remove empty boxes
            keep = box_ops.remove_small_boxes(boxes, min_size=1e-2)
            boxes, scores, labels, idx_sel = boxes[keep], scores[keep], labels[keep], idx_sel[keep]
    ...
    def forward(self,
        ...
        if self.training:
            proposals, matched_idxs, labels, regression_targets, labels_breed = self.select_training_samples(proposals, targets)
        else:
            labels = None
        ...
        class_logits, box_regression, class_logits_breed = self.box_predictor(box_features)
        ...
        if self.training:
            ...
            loss_classifier, loss_box_reg, loss_classifier_breed = fastrcnn_loss(
                class_logits, box_regression, labels, regression_targets, class_logits_breed, labels_breed)
